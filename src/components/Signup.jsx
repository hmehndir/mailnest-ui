...
const Signup = () => {
    ...
    const submitHandler = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post("https://hmail-service.vercel.app/api/v1/user/register", input, {
                headers:{
    ...
    return (
        <div className='flex items-center justify-center w-screen h-screen'>
            <form onSubmit={submitHandler} className='flex flex-col gap-3 bg-white p-4 w-[20%]'>
                ...
                <input onChange={changeHandler} value={input.fullname} name='fullname' type='text' placeholder='username' className='border border-gray-400 rounded-md px-2 py-1' />
